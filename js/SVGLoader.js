 ...
 */
let arr = [], idarr =[], pointArr = []
THREE.SVGLoader = function (type, floor) {
...
THREE.SVGLoader.prototype = {
	...
	load: function ( url, onLoad, onProgress, onError ) {
		...
		loader.load( url, function ( text ) {
			onLoad( scope.parse( text ), scope.floor) 
	...
	parse: function ( text ) {
		var that = this
		function parseNode( node, style, nihaoma ) { //解析节点   不管什么节点  全部转化为 path
			if ( node.nodeType !== 1 ) return;
			var transform = getNodeTransform( node ); //有没有tranfrom属性区别节点

			var path = null;
			let nodecname = node.getAttribute('class')
			let newArr = styleArr.filter(el=>{
				if(el.class.length < 8 && el.fill.length < 14){
					return el
				}
			})
			if(nodecname && nodecname.length < 8){
				newArr.forEach(el=>{
					if(nodecname == el.class.substring(1)){
						style.fill = '#' + el.fill.split('#')[1]
					}
				})
			}
			...
			switch ( node.nodeName ) {
				...
				case 'line':
					...
					if ( path ) {
						// arr.push(path)
						if (that.renderType == 'shop') { 
							path.shopId = node.getAttribute('id') + that.floor
						}
		...
		function parsePathNode( node ) { //解析svg path节点
		...
		*/
		...
		var curDom, curDom2, curDom3, curDom4
		...
		let textArr
		// Array.from(xml.documentElement.getElementsByTagName('g')).forEach(el=>{
		// 	// console.log(el,'el');
		// 	if(el.getAttribute('id') == 'posi'){
		...
		if(this.renderType == 'shop'){
			Array.from(xml.documentElement.getElementsByTagName('g')).forEach(el=>{
				if(el.getAttribute('id') == 'shop'){
					curDom = el
				}
			})
			let styledom = xml.documentElement.getElementsByTagName('style')[0]
			let pathdom = curDom.getElementsByTagName('path')
			let cName = pathdom[1].getAttribute('class')
			if(pathdom[1].style.cssText){
				
			}else {
				let con = xml.documentElement.textContent
				let conarr = con.split('}')
				for(let c=0;c<conarr.length-1;c++){
					let a = conarr[c].split(';')
					let b = a[0].split('{')
					let obj = {
						class: b[0],
						fill: b[1],
			...
			parseNode(curDom, {}, true);
			
			Array.from(xml.documentElement.getElementsByTagName('g')).forEach(el=>{
				if(el.getAttribute('id') == 'background'){
					curDom2 = el
				}
			})
			parseNode( curDom2, {
			...
			Array.from(xml.documentElement.getElementsByTagName('g')).forEach(el=>{
				if(el.getAttribute('id') == 'posi_icon'){
					curDom4 = el
				}
			})
			parseNode( curDom4, {
